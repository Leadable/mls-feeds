...
sub dumpAllClasses {
  ...
  foreach my $class (@$classes) {
    next if ($IGNORED_CLASSES{ $class->GetId });

    $columns->{ $resource->GetResourceID() }->{ $class->GetId() } = {};

    print "Class ID: " . $class->GetId() . "\n";
    #print "Class name: " . $class->GetClassName() . " [" .  $class->GetSystemName() . "] " . $class->GetVisibleName . "\n";
...
sub dumpAllTables {
  ...
  foreach my $table (@$tables) {
    ...
    my %info = (
      ...
      StandardName => $table->GetSystemName(),
    ...
    my $comment = qq|COMMENT ON COLUMN $MLS."| . $resource->GetResourceID() . '"."' . $table->GetSystemName() . '" IS \'' . $longname . "'";
    my $sql = qq|ALTER TABLE $MLS."| . $resource->GetResourceID() . '" ADD COLUMN "' . $table->GetSystemName() . '" ' . $DATA_TYPE_FROM_RETS_TO_PG{ $table->GetDataType() };
    ...
    $columns->{ $resource->GetResourceID() }->{ $class->GetId() }->{ $table->GetSystemName() } = \%info;
...
get '/' => sub {
  ...
  my $rets = eval {
    new librets::RetsSession("https://rets.crmls.org/contact/rets/login");
  };
  ...
  eval {
    if (!$rets->Login("LISTINGPAGES", "dri7c-Atr")) {
  ...
  $c->render(template => 'columns', mls => $MLS, columns => \%columns, lookups => \%lookups);
};
