...
sub dumpAllTables {
  ...
  foreach my $table (@$tables) {
    ...
    my $comment = 'COMMENT ON COLUMN ragfl."Property"."' . $table->GetDBName() . '" IS \'' . $longname . "'";
    my $sql = 'ALTER TABLE ragfl."Property" ADD COLUMN "' . $table->GetDBName() . '" ' . $DATA_TYPE_FROM_RETS_TO_PG{ $table->GetDataType() };
...
#my $sql = $mt->render_file('./tables.mt', 'ragfl', $metadata);
#print $sql;
...
get '/' => sub {
  ...
  $c->render(template => 'columns', mls => 'ragfl', columns => \%columns);
};
