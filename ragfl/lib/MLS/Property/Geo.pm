...
sub update_local_row {
  ...
  my $sql = "UPDATE $MLS::Property::Config::MLS.\"$MLS::Property::Config::RESOURCE\" SET " . join(', ', @vals) . " WHERE " . $dbh->quote_identifier($MLS::Property::Config::PRIMARY_KEY{DBName}) . " = " . $dbh->quote($remote_row->{remote_id});
  print "$sql\n";
  $dbh->do($sql);
}
