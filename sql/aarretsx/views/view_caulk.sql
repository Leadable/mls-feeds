...
CREATE OR REPLACE VIEW aarretsx.view_caulk AS
  SELECT
   ...
   vl.*,
   array(select jsonb_array_elements_text(__geo_places#>'{1,"Neighborhood"}')::text) as __geo_neigh,
   array(select jsonb_array_elements_text(__geo_places#>'{1,"City"}')::text) as __geo_city,
   array(select jsonb_array_elements_text(__geo_places#>'{1,"School District"}')::text) as __geo_school_district
  FROM
