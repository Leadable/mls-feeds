...
sub fetch_local {
  ...
  if (%MLS::Config::STATUS_COLUMN) {
    push @select, $dbh->quote_identifier($MLS::Config::STATUS_COLUMN{DBName});
  }

  if (%MLS::Config::PRICE_COLUMN) {
    push @select, $dbh->quote_identifier($MLS::Config::PRICE_COLUMN{DBName});
  }
...
sub update {
  ...
  if (%MLS::Config::PRICE_COLUMN) {
    my $price_val = $local_row->{ $MLS::Config::PRICE_COLUMN{DBName} };
  ...
  my $status_val = $local_row->{ $MLS::Config::STATUS_COLUMN{DBName} };
...
sub insert {
  ...
  if (%MLS::Config::PRICE_COLUMN) {
    ...
    push(@vals, 'ARRAY[' . $data->{ $MLS::Config::PRICE_COLUMN{DBName} } . '::numeric]');
  }
  ...
  if (%MLS::Config::STATUS_COLUMN) {
    ...
    push(@vals, 'ARRAY[' . $data->{ $MLS::Config::STATUS_COLUMN{DBName} } . ']');
  }
